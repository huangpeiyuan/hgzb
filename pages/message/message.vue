<template>
	...
	<view>
		<view class="search">
			...
			<!-- <view class="lgz">
				<view v-for="(item1,index1) in list1" :key="index1" v-if="lucene" style="background-color: #FFFFFF; width: 600upx;">
					<span @click="acName(item1.announcementName)">{{item1.announcementName}}</span>
				</view>
			</view> -->
		...
		<!-- <button class="search-input-button" id="btn" @click="set"><i>搜索</i></button>
				<view class="suggest">
					<ul id="search-result">
		...
		<!-- 		<view class="zb-list">
			<view v-for="(item,index) in list" :key="index" class="main-padding" @click="det" :data-announcementName = "item.announcementName" :data-addRess="item.address" :data-time = "item.time" :data-gxxText = "item.gxxText" :data-url = "item.url" :data-enclosureName = "item.enclosureName" :data-enclosureUrl="item.enclosureUrl" :data-radioFlag="item.radio_flag" :data-index="index">
				<view class="mine">
					<view class="mine-head">
						
						<span class="radio" style="width: 10upx; height: 10upx;  border-radius: 50%; background-color: red;
						   margin-top: 10upx; float: left; margin-right: 10upx;" v-if="item.radio_flag"></span>

						<view>{{item.announcementName}} {{item.radio_flag}}</view>
					</view>
					<view class="mine-body">
						<span class="city_bg">{{item.address}}</span>
		...
		<view class="zb-list">
			<view v-for="(item,index) in list" :key="index" class="main-padding" :data-id="item.id" :data-title="item.title"
			 :data-cityName="item.cityName" :data-reportDate="item.reportDate" :data-detail="item.detail" :data-originUrl="item.originUrl"
			 :data-attachedName="item.attachedName" :data-attachedUrl="item.attachedUrl" :data-radioFlag="item.radio_flag"
			 :data-index="index" @click="det">
				<view class="mine">
					...
					<view class="mine-body">
						<view class="city_bg">{{null != item.cityName?item.cityName:item.provinceName}}</view>
						<view class="type_bg">{{item.jobType}}</view>
						<view class="time_bg">{{item.reportDate}}</view>
					</view>
				</view>
			</view>
		</view>
		...
		<!-- <view class="box2" v-for="(item,index) in list" :key="index">
			...
			<p class="name_bg" @click="det(item.announcementName,item.address,item.time,item.gxxText,item.url,
			   item.enclosureName,item.enclosureUrl)">{{item.announcementName}}</p>


			<span class="city_bg">{{item.administrative}}</span>
			<span class="type_bg">{{item.announcementType}}</span>
			<span class="time_bg">{{item.time}}</span>
	...
	export default {
		...
		methods: {
			login(e) {
				...
				uni.login({
					success(response) {
						...
						me.popupFlag = false
						uni.request({
							url: "https://haogz8.com:2443/recruit_brother/wechatAppletsUser/login",
							method: "POST",
							data: {
								"avatarUrl": avatarUrl,
								"nickName": nickName,
			...
			async sear() {
				...
				const res = await this.$myRequest({
					url: '//recruit_brother/recruit/findTenData?provinceCode=' + this.provinceValue + '&cityCode=' + this.cityValue +
