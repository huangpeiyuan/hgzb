<template>
	<view class="vf">
		<view class="header">
			...
			<p class="pubTime">
				<span>地区：{{null != cityName?cityName:provinceName}} &nbsp; &nbsp;时间：{{reportDate}}</span>
			</p>
		</view>

		<view class="content" v-html="detail" v-if="detail != 'null' && detail != 'undefined' ? true : false">
		</view>
		...
		<view class="enclosure" v-show="null != attachedName" v-for="(attachedName, index) in attachedNameList">
			附件地址：<navigator :url="enclosureUrl[index]">{{attachedName}}</navigator>
	...
	export default {
		data() {
			return {
				...
				cityCode: null,
				jobType: "",
				attachedNameList: null,
				attachedURLList: null,
			}
		},
		onLoad: function(options) {
			...
			uni.request({
				url: "https://haogz8.com:2443/recruit_brother/recruit/findById",
				method: "GET",
				data: {
					id: options.id,
				...
				success(res) {
					...
					_this.url = data.originUrl;
					_this.attachedName = data.attachedName;
					_this.enclosureUrl = data.attachedURL;
					_this.codeId = data.codeId;
					_this.hide();
				},
			})
			console.log("codeId1111" + this.codeId)
			var arr = []
		...
		methods: {
			...
			addCollection() {
				...
				uni.request({
					url: "https://haogz8.com:2443/recruit_brother/collection/insertCollection?",
					method: "GET",
					data: {
