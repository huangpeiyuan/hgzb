...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
			var me = this
			return {
				title: "好工作吧，每天免费给用户提供10000+条政府企事业单位的工作岗位工作数据",
						...
						success(res) {
							...
							if (res.data.code === 0) {
								...
								if (res.data.data.subscribe) {
									...
									uni.request({
										url: "https://haogz8.com:2443/recruit_brother/collection/findCollection?",
										method: "GET",
										data: {
		...
		methods: {
			...
			async get() {
				...
				const res = await this.$myRequest({
					url: 'https://haogz8.com:2443/recruit_brother/historyCollection/findHistory?user_id' + UserInfo.open
				})
